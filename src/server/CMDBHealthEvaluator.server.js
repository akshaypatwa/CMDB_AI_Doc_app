...
CMDBHealthEvaluator.prototype = {
    initialize: function () {
        ...
        this.W_CORRECTNESS = 0.4
        this.W_COMPLETENESS = 0.3
        this.W_COMPLIANCE = 0.3

        // GOVERNANCE from System Properties
        this.governance = {
            restricted_fields: gs
                .getProperty('cmdb_health.governance.restricted_fields', 'owned_by,business_criticality')
                .split(','),
            safe_autofix_fields: gs
                .getProperty('cmdb_health.governance.safe_autofix_fields', 'operational_status')
                .split(','),
    ...
    evaluate: function (ciSysId) {
        ...
        gs.info(
            '[CMDBHealthEvaluator] CI loaded: ' +
                ci.getDisplayValue('name') +
                ' (' +
                ciClass +
                ') — starting Phase 1A: Completeness'
        )

        // Run sections in order — completeness first (its score feeds correctness)
        var completenessData = this._runCompleteness()
        gs.info(
            '[CMDBHealthEvaluator] Phase 1A Completeness done — score: ' +
                completenessData.score +
                ', missing fields: ' +
                completenessData.missing_count +
                '/' +
                completenessData.recommended_count +
                ' — starting Phase 1B: Correctness'
        )

        var correctnessData = this._runCorrectness(completenessData.score)
        gs.info(
            '[CMDBHealthEvaluator] Phase 1B Correctness done — quality score: ' +
                correctnessData.quality_score.total +
                ', stale: ' +
                correctnessData.is_stale +
                ', orphan: ' +
                correctnessData.is_orphan +
                ' — starting Phase 1C: Compliance'
        )

        var complianceData = this._runCompliance()
        gs.info(
            '[CMDBHealthEvaluator] Phase 1C Compliance done — score: ' +
        ...
        var overallScore = complianceData.has_regulatory ? Math.min(rawOverall, 40) : rawOverall

        var healthStatus =
            overallScore >= 80 ? 'healthy' : overallScore >= 60 ? 'minor' : overallScore >= 40 ? 'moderate' : 'critical'
        ...
        gs.info(
            '[CMDBHealthEvaluator] Evaluation COMPLETE for CI: ' +
                ciSysId +
                ' — overall: ' +
                overallScore +
                ', status: ' +
    ...
    _runCompleteness: function () {
        gs.info(
            '[CMDBHealthEvaluator] [' +
                this.ciSysId +
                '] Completeness — querying cmdb_recommended_fields for class: ' +
                this.ciClass
        )
        ...
        if (recRec.next()) {
            ...
            gs.warn(
                '[CMDBHealthEvaluator] [' +
                    this.ciSysId +
                    '] Completeness — no recommended fields record found for class: ' +
                    this.ciClass +
                    '. Score will default to 100.'
            )
        }
        ...
        var completenessScore =
            recommendedFields.length > 0 ? Math.round((populatedFields.length / recommendedFields.length) * 100) : 100
        ...
        gs.info(
            '[CMDBHealthEvaluator] [' +
                this.ciSysId +
                '] Completeness — running peer analysis for ' +
                missingFields.length +
                ' missing field(s)'
        )
        for (var pi = 0; pi < missingFields.length; pi++) {
            ...
            gs.info(
                '[CMDBHealthEvaluator] [' +
                    this.ciSysId +
                    '] Completeness — peer analysis for field "' +
                    pField +
                    '": peers=' +
                    entry.peer_total_raw +
                    ', consensus=' +
                    entry.consensus_type +
                    ', top_value=' +
                    entry.top_value +
                    ' (' +
                    entry.top_value_pct +
    ...
    _runCorrectness: function (completenessScore) {
        gs.info(
            '[CMDBHealthEvaluator] [' +
                this.ciSysId +
                '] Correctness — checking discovery staleness (threshold: ' +
        ...
        gs.info(
            '[CMDBHealthEvaluator] [' +
                this.ciSysId +
                '] Correctness — last_discovered: ' +
                (lastDiscVal || 'NULL') +
                ', age: ' +
                (discoveryAge !== null ? discoveryAge + ' days' : 'unknown') +
                ', stale: ' +
                isStale
        )
        ...
        gs.info(
            '[CMDBHealthEvaluator] [' +
                this.ciSysId +
                '] Correctness — relationships: as_parent=' +
                relationships.as_parent +
                ', as_child=' +
                relationships.as_child +
                ', total=' +
                relationships.total +
                ', orphan=' +
                relationships.is_orphan
        )

        gs.info(
            '[CMDBHealthEvaluator] [' +
                this.ciSysId +
                '] Correctness — querying incident count (lookback: ' +
                this.INCIDENT_LOOKBACK_DAYS +
                ' days)'
        )
        ...
        gs.info(
            '[CMDBHealthEvaluator] [' +
                ...
                qualityScore.components.completeness.score +
                ', discovery_component=' +
                qualityScore.components.discovery.score +
                ', relationships_component=' +
                qualityScore.components.relationships.score +
                ', incidents_component=' +
                qualityScore.components.incidents.score +
                ')'
        )
        ...
        gs.info(
            '[CMDBHealthEvaluator] [' +
                this.ciSysId +
                '] Correctness — running duplicate detection on identifiers: [' +
                this.DUPLICATE_IDENTIFIERS.join(', ') +
                ']'
        )
        ...
        gs.info(
            '[CMDBHealthEvaluator] [' +
                this.ciSysId +
                '] Correctness — duplicates: total_found=' +
                duplicateData.total_found +
                ', qualified=' +
                duplicateData.qualified_count +
    ...
    _runCompliance: function () {
        gs.info(
            '[CMDBHealthEvaluator] [' +
                this.ciSysId +
                '] Compliance — querying cert_template for class: ' +
                this.ciClass
        )
        var templates = []
        ...
        while (tmplRec.next()) {
            ...
            gs.info(
                '[CMDBHealthEvaluator] [' +
                    this.ciSysId +
                    '] Compliance — found template: "' +
                    tmplRec.getDisplayValue('name') +
                    '" (regulatory: ' +
                    tmplIsReg +
                    ')'
            )
        ...
        if (templates.length === 0) {
            gs.info(
                '[CMDBHealthEvaluator] [' +
                    this.ciSysId +
                    '] Compliance — no cert_template records found for class: ' +
                    this.ciClass +
            ...
            gs.info(
                '[CMDBHealthEvaluator] [' +
                    this.ciSysId +
                    '] Compliance — evaluating rules across ' +
                    templates.length +
                    ' template(s)'
            )
        }
        ...
        for (var t = 0; t < templates.length; t++) {
            ...
            gs.info(
                '[CMDBHealthEvaluator] [' +
                    ...
                    tmplSkipped +
                    ', violations_so_far=' +
                    Object.keys(violationMap).length
            )
        }
        ...
        for (var vKey in violationMap) {
            ...
            mv.priority_level = mv.is_regulatory ? 'CRITICAL' : mv.templates_affected.length > 1 ? 'HIGH' : 'MEDIUM'
        ...
        for (var vi = 0; vi < mergedViolations.length; vi++) {
            ...
            gs.info(
                '[CMDBHealthEvaluator] [' +
                    this.ciSysId +
                    '] Compliance — violation: field="' +
                    vField +
                    '", type=' +
                    mergedViolations[vi].violation_type +
                    ', priority=' +
                    mergedViolations[vi].priority_level +
                    ', actual="' +
                    mergedViolations[vi].actual_value +
                    '"' +
                    ', expected="' +
                    mergedViolations[vi].expected_value +
                    '"'
            )
        }

        var complianceScore = totalRulesEval > 0 ? Math.round((rulesPassed / totalRulesEval) * 100) : 100

        var hasCritical = false
        for (var ci2 = 0; ci2 < mergedViolations.length; ci2++) {
            if (mergedViolations[ci2].priority_level === 'CRITICAL') {
                hasCritical = true
                break
            }
        }
        ...
        gs.info(
            '[CMDBHealthEvaluator] [' +
                ...
                skippedCount +
                ', violations=' +
                mergedViolations.length +
                ', has_regulatory_violation=' +
                hasCritical +
                ', score=' +
                complianceScore
        )
    ...
    _runPeerQueryWithFallback: function (field) {
        ...
        for (var s = 0; s < steps.length; s++) {
            ...
            if (result.peerTotalRaw >= this.MIN_PEER_SAMPLE) {
                return {
                    data: result,
                    class_used: steps[s].cls,
                    env_scoped: steps[s].envScoped,
                    label: steps[s].label,
                    fallback_used: s > 0,
                    fallback_step: s,
                }
        ...
        return {
            data: lastResult,
            class_used: lastStep.cls,
            env_scoped: false,
            label: 'exhausted_all_fallbacks',
            fallback_used: true,
            fallback_step: steps.length,
        }
    },
    ...
    _runPeerQuery: function (field, queryClass, envValue) {
        ...
        var normValues = []
        for (var key in normMap) {
            normValues.push(normMap[key])
            normTotal += normMap[key].count
        }
        normValues.sort(function (a, b) {
    ...
    _buildPeerEntry: function (field, qr) {
        ...
        var topPct = 0
        var conType = 'UNKNOWN'
        var confLevel = 'INSUFFICIENT_DATA'
        if (pData.peerTotalRaw >= this.MIN_PEER_SAMPLE && topVals.length > 0) {
            topPct = (topVals[0].count / pData.normalizedTotal) * 100
            conType =
                topPct >= this.HIGH_CONFIDENCE_PCT
                    ? 'STRONG_STANDARD'
                    : topPct <= this.LOW_CONFIDENCE_PCT
                      ? 'STATISTICAL_NOISE'
                      : 'MEDIUM_CONFIDENCE'
            confLevel =
                pData.peerTotalRaw > 50 ? 'HIGH_SAMPLE' : pData.peerTotalRaw >= 15 ? 'MEDIUM_SAMPLE' : 'LOW_SAMPLE'
        }
        var entry = {
            field: field,
    ...
    _findDuplicates: function (targetQualityTotal, isStale) {
        ...
        for (var i = 0; i < this.DUPLICATE_IDENTIFIERS.length; i++) {
            ...
            if (!this._isValidIdentifier(val)) {
                gs.info(
                    '[CMDBHealthEvaluator] [' +
                        this.ciSysId +
                        '] Duplicates — skipping identifier "' +
                        fld +
                        '" (blank or junk value)'
                )
                continue
            ...
            gs.info(
                '[CMDBHealthEvaluator] [' +
                    this.ciSysId +
                    '] Duplicates — checking identifier "' +
                    fld +
                    '" = "' +
                    val +
                    '" against cmdb_ci INSTANCEOF ' +
                    this.ciClass
            )
        ...
        return {
            total_found: matchOrder.length,
            qualified_count: scored.length,
            truncated: scored.length > this.DUPLICATE_DETAIL_LIMIT,
            detail_limit: this.DUPLICATE_DETAIL_LIMIT,
            top_duplicates: topDups,
        }
    },
    ...
    _buildDupSignals: function (record) {
        ...
        var ldv = record.getValue('last_discovered')
        var dAge = null
        if (ldv) {
            var n2 = new GlideDateTime()
            var l2 = new GlideDateTime(ldv)
            dAge = this._daysBetween(n2, l2)
        }
        ...
        var sigs = {
            name: record.getDisplayValue('name'),
            sys_id: sid,
            ...
            relationships: rels,
            incident_count: incs,
            quality_score: dupQuality.total,
        }
    ...
    _getClassFallbackChain: function (startClass) {
        ...
        gs.info(
            '[CMDBHealthEvaluator] [' + this.ciSysId + '] Class hierarchy fallback chain: [' + chain.join(' → ') + ']'
        )
        return chain
    },
    ...
    _getIncidentCount: function (sysId) {
        ...
        cutoff.addDaysUTC(-this.INCIDENT_LOOKBACK_DAYS)
        var agg = new GlideAggregate('incident')
        agg.addAggregate('COUNT')
        agg.addQuery('cmdb_ci', sysId)
        agg.addQuery('sys_created_on', '>=', cutoff)
        agg.query()
    ...
    _calcQualityScore: function (compScore, discAge, relTotal, incCnt) {
        ...
        return {
            total: c + d + r + inc,
            components: {
                completeness: { score: c, max: 40 },
                discovery: { score: d, max: 30 },
                relationships: { score: r, max: 20 },
                incidents: { score: inc, max: 10 },
            },
        }
    },
    ...
    _getSuggestedRange: function (opCode, desiredValue) {
        var val = parseFloat(desiredValue)
    ...
    _evaluateRule: function (fieldName, opCode, desiredValue) {
        ...
        var actualNum = parseFloat(actualRaw)
        var desiredNum = parseFloat(desiredValue)
