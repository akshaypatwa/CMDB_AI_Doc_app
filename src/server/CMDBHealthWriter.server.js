    ...
    save: function(ciSysId, mergedPayload, llmResponse, targetStatus) {
        ...
        try {
            ...
            if (mergedPayload) {
                ...
                gr.setValue(this.F + 'has_duplicates',       (correct.duplicates && correct.duplicates.qualified_count > 0) || false);
            ...
            if (llmResponse) {
                ...
                var highRisk = 0;
                for (var i = 0; i < reviewItems.length;  i++) { if (reviewItems[i].risk  === 'HIGH') highRisk++; }
                for (var j = 0; j < autofixItems.length; j++) { if (autofixItems[j].risk === 'HIGH') highRisk++; }
            ...
            if (status === 'evaluated') {
                ...
                gs.info('[CMDBHealthWriter] CI ' + ciSysId + ' — Phase 1 DONE. Scores: overall=' +
            ...
            gs.error('[CMDBHealthWriter] save() failed for CI ' + ciSysId + ' (status=' + targetStatus + '): ' + e.message);
            return false;
        }
    },
