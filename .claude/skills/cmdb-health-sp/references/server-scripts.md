# Server Scripts (ES5 — var only)

All server scripts use ES5. No `let`, `const`, arrow functions, or template literals.

---

## Widget 1: Header Summary (`cmdb_health_header`)

Aggregates counts by status and fetches last completed run time.

```javascript
(function() {
    data.summary = { total: 0, critical: 0, moderate: 0, minor: 0, healthy: 0, lastRun: '', hasCritical: false };

    // Aggregate counts by health_status
...
## Widget 2: Load All Health Records
...
## Widget 2: Add CI Action Handler
